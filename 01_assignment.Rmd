...
hotels %>% 
  ...
  ggplot(aes(y = previous_cancellations))+
  geom_bar()
```
...
hotel_recipe <- recipe(is_canceled ~ . , data = hotel_training) %>% 
  step_mutate(children = ifelse(children == 0,0,1),
  babies = ifelse(babies == 0,0,1),
  previous_cancelations = ifelse(previous_cancellations == 0,0,1),
...
```{r}
hotel_lasso <- linear_reg(mixture = 1) %>% 
  set_engine("glmnet") %>% 
